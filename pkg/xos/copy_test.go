...
func Test_CreateDir(t *testing.T) {
	...
	paths := []string{
		filepath.Join(tmp_fs, "dir_3"),
		filepath.Join(tmp_fs, "dir_4", "dir_41"),
		filepath.Join(tmp_fs, "dir_5", "dir_51", "dir_511"),
		filepath.Join(tmp_fs, "dir_5", "dir_51", "dir_512"),
		// duplicates entries
		filepath.Join(tmp_fs, "dir_5", "dir_51", "dir_511"),
		filepath.Join(tmp_fs, "dir_5", "dir_51", "dir_512"),
	}
	for _, p := range paths {
		if err := CreateDir(p, os.ModePerm); err != nil {
			t.Errorf("Error!! invalid existing path status \"%s\"", p)
		}
	}
}
