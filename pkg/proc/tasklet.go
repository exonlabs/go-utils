...
type TaskletHandler struct {
	...
	// determine if first time execution or not
...
// IsFirstRun returns whether the tasklet is running for first time or not.
func (h *TaskletHandler) IsFirstRun() bool {
	return h.isFirstRun.Load()
}
